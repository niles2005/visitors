(function() {
    mapwork.TrafficTileLayer = TrafficTileLayer;
    
    TrafficTileLayer.ID = "trafficTileLayer";
    var EXTEND = mapwork.Layer;
    var TileStore = mapwork.TileStore;
    
    function TrafficTileLayer(layerId) {
        if(EXTEND) {
        ...
        if(layerId) {
            ...
            this._layerId = TrafficTileLayer.ID;
        }
        ...
        this._urlPath = mapwork.configs[this._layerId + "_url"] + "&dataset=" + mapwork.mapStyleT;
        this._grid = [];
    }
...
//            console.log("initLayer");
        ...
        makeTileUrl: function(x,y,zoom) {
            if(this._urlPath) {
                return this._urlPath + "&x=" + x + "&y=" + y + "&zoom=" + zoom + "&t=" + (new Date()).getTime();
		...
		refreshPage: function() {
            for(var k in this._grid) {
                var row = this._grid[k];
                for(var j in row) {
                    row[j]._image.style.display="none";
                    row[j].draw();
                }
            }
		}
    ...
    if(EXTEND) {
        mapwork.utils.inherits(TrafficTileLayer, EXTEND);
    }
})();